...
class PA_En_Module(nn.Module):
    def __init__(self,attention_size):
        ...
        self.FC_2 = nn.Linear(attention_size//16,attention_size//4)
    def forward(self, h):
...
class Classic_JSCC(nn.Module):
    def __init__(self,args):
        ...
        if self.PA_flag==1:
            self.En_PA=PA_En_Module(attention_size*args.S)
            self.De_PA=PA_De_Module(attention_size*args.S)
    ...
    def compute_h_broadcast(self,batch_size,subchannel_num):
        ...
        if (self.PA_flag or self.tran_know_flag)==1:
            h_norm=torch.abs(h)
